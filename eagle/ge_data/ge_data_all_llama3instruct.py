...
parser.add_argument('--data_path', type=str, default='./data/ShareGPT_V4.3_unfiltered_cleaned_split.json')
...
bigname="meta-llama/Llama-3.2-3B-Instruct"
...
def build_dataset_rank(
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples['id'])):
            ...
            human_sep = conv.sep + conv.roles[0] + ": "
            assistant_sep = conv.sep + conv.roles[1] + ": "

            turns = conversation.split(human_sep)
            cur_len = len(tokenizer(turns[0] + human_sep).input_ids) - 4
            for j, turn in enumerate(turns[1:]):
                if turn == "":
                    break

                turn_len = len(tokenizer(turn + human_sep).input_ids) - 2
                parts = turn.split(assistant_sep)
                assistant_start_idx = len(tokenizer(parts[0] + assistant_sep).input_ids) + 1

                # print(f"{j}======")
                # print(tokenizer.decode(input_ids[cur_len: cur_len + 3], skip_special_tokens=False))
                # print(tokenizer.decode(input_ids[cur_len + assistant_start_idx - 3: cur_len + assistant_start_idx], skip_special_tokens=False))
                # print(tokenizer.decode(input_ids[cur_len + assistant_start_idx: cur_len + assistant_start_idx + 5], skip_special_tokens=False))
                # if j >= 20:
                #     break

                loss_mask[cur_len + assistant_start_idx: cur_len + turn_len] = 1
            ...
            # import numpy as np
            # decode_text = tokenizer.decode(np.where(loss_mask == 1, input_ids, tokenizer.bos_token_id), skip_special_tokens=False)
            # print(decode_text)
...
for id,data in enumerate(tqdm(ds, desc="ge", total=ds.num_rows)):
    outdata = ge(data)
